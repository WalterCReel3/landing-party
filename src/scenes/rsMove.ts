...
import { getPath, getValid } from "../pathfinding";
    ...
    create(): void {
        ...
        this.input.on('pointerdown', (pointer, gameObject) => {
            ...
            if (this.isAtPlayerCoords(tileCoords)) {
                ...
                let limitDistance = 2;
                let set = getValid({board: board, position:[tileCoords.x,tileCoords.y], limitDistance});
                set.forEach(([x, y]) => {
                    this.spawnOrderTargetButton(tileCoords, new Vector2(x, y));
                });
            }
